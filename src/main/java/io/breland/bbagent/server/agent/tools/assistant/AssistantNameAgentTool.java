...
public class AssistantNameAgentTool implements ToolProvider {
  ...
  public AgentTool getTool() {
    return new AgentTool(
        TOOL_NAME,
        "Store, set, or forget a user's name for global use across chats. Only store a name after"
            + " the user explicitly agrees and mention you will use it across any chats the user is"
            + " present in.",
        jsonSchema(AssistantNameRequest.class),
        false,
        (context, args) -> {
          String sender = context.sender();
          if (sender == null) {
            return "no sender";
          }
          ...
          if (action == Action.FORGET) {
            context.removeGlobalNameForSender(sender);
          ...
          context.setGlobalNameForSender(sender, name);
