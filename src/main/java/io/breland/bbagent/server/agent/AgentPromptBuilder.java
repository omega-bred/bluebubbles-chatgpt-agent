    ...
    return EasyInputMessage.builder()
        ...
        .content(
            "You may respond with plain text if that is sufficient. "
                ...
                + "For dining room LED wall or wallart requests, search for the wallart tools: getCurrentArt sends the current artwork as a chat photo, showImage displays an attached photo unchanged, composeArt edits or combines image references with a prompt, and showNewArt generates new art. Use source=current_art to edit the current wall image and source=attachment for chat photos; never put image bytes in tool arguments. Art submissions are asynchronous: use getArtStatus before claiming deployment finished. "
