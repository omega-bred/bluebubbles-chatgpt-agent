...
public class GetWebsiteAccountLinkStatusAgentTool implements ToolProvider {
    ...
    return new AgentTool(
        ...
        (context, args) -> {
          try {
            ...
            String sender =
                firstWithText(request.sender(), message == null ? null : message.sender());
            String chatGuid =
                firstWithText(request.chatGuid(), message == null ? null : message.chatGuid());
  ...
  private String userFacingText(WebsiteAccountService.SenderLinkStatus status) {
    if (!StringUtils.hasText(status.accountBase())) {
    ...
    if (status.linked()) {
      return "This iMessage sender is linked to a web account, but this specific chat does not"
          + " have its own web account link yet. "
          + modelText(status);
    }
  ...
  private String firstWithText(String... values) {
