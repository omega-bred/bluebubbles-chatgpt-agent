...
public class StartCoderAsyncTaskAgentTool implements ToolProvider {
    ...
    try {
      ...
      CoderAsyncTaskStartStore.Reservation reservation =
          taskStartStore.reserve(
              idempotencyKey,
              accountBase,
              context.message() == null ? "" : defaultIfBlank(context.message().chatGuid(), ""),
              context.message() == null ? null : context.message().messageGuid(),
  ...
  private String replayExistingStart(CoderAsyncTaskStartEntity entity, ObjectMapper mapper)
    ...
    if (CoderAsyncTaskStartStore.STATUS_FAILED.equals(entity.getStatus())) {
      response.put("error", defaultIfBlank(entity.getErrorMessage(), "previous start failed"));
      response.put(
          "message",
  ...
  private TemplateSelection selectTemplate(
    ...
    if (!isBlank(templateVersionId)) {
      return new TemplateSelection(
          templateVersionId.trim(), defaultIfBlank(templateName, "provided"));
    }
    ...
    return new TemplateSelection(
        ...
        defaultIfBlank(text(selected, "display_name", "displayName"), text(selected, "name")));
  }
  ...
  private int scoreTemplate(JsonNode template, @Nullable String preferredName) {
    String name = lowerCase(text(template, "name"), Locale.ROOT);
    String displayName = lowerCase(text(template, "display_name", "displayName"), Locale.ROOT);
    String description = lowerCase(text(template, "description"), Locale.ROOT);
    String id = lowerCase(text(template, "id"), Locale.ROOT);
    String activeVersionId =
        lowerCase(text(template, "active_version_id", "activeVersionId"), Locale.ROOT);
    String haystack = String.join(" ", name, displayName, description, id, activeVersionId);
    if (!isBlank(preferredName)) {
      String preferred = lowerCase(preferredName, Locale.ROOT);
  ...
  private String templateSummaries(List<JsonNode> templates) {
    ...
    for (JsonNode template : templates) {
      summaries.add(
          defaultIfBlank(text(template, "display_name", "displayName"), text(template, "name"))
              + " (active_version_id="
  ...
  private static String summarize(String text) {
    return abbreviate(defaultString(normalizeSpace(text)), 120);
  }
  ...
  private static String idempotencyKey(
    ...
    String naturalKey =
        !isBlank(messageGuid)
            ? accountBase + "|" + defaultIfBlank(chatGuid, "") + "|" + messageGuid
            : accountBase
                + "|"
                + defaultIfBlank(chatGuid, "")
                + "|"
  ...
  private static String normalizeTask(String task) {
    return lowerCase(defaultString(normalizeSpace(task)), Locale.ROOT);
  }
